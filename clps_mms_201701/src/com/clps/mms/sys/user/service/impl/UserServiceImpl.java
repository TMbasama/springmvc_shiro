	...
	public boolean register(UserInfo user) throws UserServiceException {
		try {
			if (this.findUserInfoByName(user.getName())!=null) {
				...
				if (userDao.insertUserInfo(user)) {
					LogModel model=new LogModel();
					model.setCreateName(user.getCreatename());
					model.setCreateDate(user.getCreatedate());
					model.setOperateMenu("用户管理");
					model.setOperateName(user.getName());
					model.setResourceName("用户");
	...
	public boolean updateUserInfoById(UserInfo userInfo) throws UserServiceException {
        try {
				...
				if (null!=dbUserInfo) {
					...
					LogModel model=new LogModel();
					model.setCreateName(userInfo.getCreatename());
					model.setCreateDate(userInfo.getCreatedate());
					model.setOperateMenu("用户管理");
					model.setOperateName(userInfo.getName());
					model.setResourceName("用户");
					...
					if (userInfo.getMobNum()!=null) {
						map.put("mobile", userInfo.getMobNum());
					}
